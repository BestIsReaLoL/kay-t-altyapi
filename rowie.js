...
client.on("guildMemberAdd", async member => {
    moment.locale("tr");
    let kanal = client.channels.cache.get("971485414215008366");
    await kanal
      .send(
   ...
   Sunucu kurallarımız <#971473784794153001>  kanalında belirtilmiştir. Unutma sunucu içerisinde ki ceza işlemlerin kuralları okuduğunu varsayarak gerçekleştirilecek. 
  ...
  client.on("guildMemberAdd", member => {
    let botrolü = "971473784261443689"
    let kayıtsızrolü = "971473784139837554";
    if (member.user.bot) {
...
client.on("userUpdate", async (oldUser, newUser) => {
  ...
  const tag = '✦'
  const sunucu = '971473784118849546'
  const log = '971473785813348371'
  const rol = '971473784169168971'
